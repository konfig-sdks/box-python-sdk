# coding: utf-8

"""
    Box Platform API

    [Box Platform](https://box.dev) provides functionality to provide access to content stored within [Box](https://box.com). It provides endpoints for basic manipulation of files and folders, management of users within an enterprise, as well as more complex topics such as legal holds and retention policies.

    The version of the OpenAPI document: 2.0.0
    Contact: devrel@box.com
    Created by: https://box.dev
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel

from box_python_sdk.pydantic.file_mini import FileMini
from box_python_sdk.pydantic.file_version_mini import FileVersionMini
from box_python_sdk.pydantic.legal_hold_policy_assignment import LegalHoldPolicyAssignment

class FileVersionLegalHold(BaseModel):
    # The unique identifier for this file version legal hold
    id: typing.Optional[str] = Field(None, alias='id')

    # `file_version_legal_hold`
    type: typing.Optional[Literal["file_version_legal_hold"]] = Field(None, alias='type')

    file_version: typing.Optional[typing.Union[bool, date, datetime, dict, float, int, list, str, None]] = Field(None, alias='file_version')

    file: typing.Optional[typing.Union[bool, date, datetime, dict, float, int, list, str, None]] = Field(None, alias='file')

    # List of assignments contributing to this Hold.
    legal_hold_policy_assignments: typing.Optional[typing.List[LegalHoldPolicyAssignment]] = Field(None, alias='legal_hold_policy_assignments')

    # Time that this File-Version-Legal-Hold was deleted.
    deleted_at: typing.Optional[datetime] = Field(None, alias='deleted_at')
    class Config:
        arbitrary_types_allowed = True

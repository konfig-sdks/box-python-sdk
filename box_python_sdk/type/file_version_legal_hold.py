# coding: utf-8

"""
    Box Platform API

    [Box Platform](https://box.dev) provides functionality to provide access to content stored within [Box](https://box.com). It provides endpoints for basic manipulation of files and folders, management of users within an enterprise, as well as more complex topics such as legal holds and retention policies.

    The version of the OpenAPI document: 2.0.0
    Contact: devrel@box.com
    Created by: https://box.dev
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from box_python_sdk.type.file_mini import FileMini
from box_python_sdk.type.file_version_mini import FileVersionMini
from box_python_sdk.type.legal_hold_policy_assignment import LegalHoldPolicyAssignment

class RequiredFileVersionLegalHold(TypedDict):
    pass

class OptionalFileVersionLegalHold(TypedDict, total=False):
    # The unique identifier for this file version legal hold
    id: str

    # `file_version_legal_hold`
    type: str

    file_version: typing.Union[bool, date, datetime, dict, float, int, list, str, None]

    file: typing.Union[bool, date, datetime, dict, float, int, list, str, None]

    # List of assignments contributing to this Hold.
    legal_hold_policy_assignments: typing.List[LegalHoldPolicyAssignment]

    # Time that this File-Version-Legal-Hold was deleted.
    deleted_at: datetime

class FileVersionLegalHold(RequiredFileVersionLegalHold, OptionalFileVersionLegalHold):
    pass

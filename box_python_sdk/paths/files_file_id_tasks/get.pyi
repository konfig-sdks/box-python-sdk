...
from box_python_sdk.model.tasks import Tasks as TasksSchema

from box_python_sdk.type.tasks import Tasks
...
from box_python_sdk.pydantic.tasks import Tasks as TasksPydantic
...
SchemaFor200ResponseBodyApplicationJson = TasksSchema
...
class ListOnFileRaw(BaseApi):
    # this class is used by api classes that refer to endpoints with operationId fn names

    async def alist_on_file(
        self,
        file_id: str,
        **kwargs,
    ) -> typing.Union[
        ...
        args = self._list_on_file_mapped_args(
            file_id=file_id,
        )
        return await self._alist_on_file_oapg(
    ...
    def list_on_file(
        ...
        return self._list_on_file_oapg(
            path_params=args.path,
        )

class ListOnFile(BaseApi):
    ...
    async def alist_on_file(
        ...
        **kwargs,
        ...
        raw_response = await self.raw.alist_on_file(
            file_id=file_id,
            **kwargs,
        )
        if validate:
            return TasksPydantic(**raw_response.body)
        return api_client.construct_model_instance(TasksPydantic, raw_response.body)
    ...
    def list_on_file(
        ...
        raw_response = self.raw.list_on_file(
            file_id=file_id,
        )
        if validate:
            return TasksPydantic(**raw_response.body)
